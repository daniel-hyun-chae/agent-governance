...
This is a governance workspace. All product artifacts live under `project/`. Every file path in this prompt that refers to product content is relative to `project/`. When executing shell commands (build, test, lint), use the `workdir` parameter set to `project/` (or the absolute path to the project directory).

## Override loading

At the start of every session, check whether `project/.opencode-overrides/agents/change-orchestrator.override.md` exists. If it does, read it and apply its contents:

- **Test commands**: use the override's validation and test commands instead of the generic preference order.
- **Product-specific testing patterns**: apply any additional testing checklists from the override alongside the framework testing patterns.

If the override file does not exist, use the generic preference order defined in the Testing section below.

## Core workflow

1. Initiation: when the user speaks, determine the work source:
   - If the user references a backlog item (e.g. "implement BG-005"), read that item from `project/backlog/proposed/` or `project/backlog/done/` for scope and context.
...
5. Implementation gate: only when the user says implement or /implement:
   - Update the backlog item file:
     ...
     - Append the Implementation Plan, Task List, Implementation Notes, and Tests sections (see `project/backlog/README.md` template).
     - The Task List starts with unchecked items derived from the plan.
   - For ad-hoc non-trivial work without an existing item:
     - Read `project/backlog/README.md` for the next available ID.
     - Create a new `BG-NNN-slug.md` file in `project/backlog/proposed/` with planning sections filled and implementation sections appended, status set to `In Progress`.
     - Increment the next ID in `project/backlog/README.md`.
...
7. Testing and validation: always run CI-equivalent local validation after implementation.
   - If the override file defines test commands, run those from `project/` (use `workdir` parameter).
   - Otherwise, use the generic preference order:
...
8. Completion:
   - Update the relevant `project/spec/` file(s) to reflect new or changed behaviors. If the item introduces a new domain, create a new `project/spec/<domain>.md` file. The backlog item's `Spec:` field indicates which sections to update.
   - Set status to `Done` in the backlog item.
   - Move the file from `project/backlog/proposed/` to `project/backlog/done/`.
   - Update the status in the `project/backlog/README.md` summary table.
   - If directories or module-level files were added, removed, or renamed, update `project/architecture/codebase-map.md`.
   - If new domain concepts were introduced or existing definitions changed, update `project/architecture/domain-glossary.md`.
